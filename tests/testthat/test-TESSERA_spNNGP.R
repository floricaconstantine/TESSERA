...
test_that("check_inputs_TESSERA_spNNGP validates required spNNGP fields",
          {
            expect_silent(check_inputs_TESSERA_spNNGP(mock_sp_data))
            
            broken_data <- mock_sp_data
            broken_data$coords_list <- NULL
            expect_error(check_inputs_TESSERA_spNNGP(broken_data))
          })
