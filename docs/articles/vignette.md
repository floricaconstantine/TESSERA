...
We demonstrate the workflow using a pre-processed kidney dataset
([Abedini et al. 2024](#ref-abedini2024single)), which contains 12,511
genes across 37,143 cells. This dataset is stored as a
...
We examine the number of cells per sample. Each sample contains several
thousand cells.
...
```
    ...
    > 
    > HK2873_ST HK2770_ST HK2671_ST HK3035_ST HK2852_ST HK2871_ST HK2844_ST HK2877_ST 
    >      1411      1736      2013      2027      2262      2404      2445      2960 
...
```
    ...
    > 
    >                     RBC                Endo_Lym Dedifferentiated_Tubule 
    >                      24                      84                     221 
    >                   M_TAL                  Endo_G        Endo_Peritubular 
    >                     334                     359                     516 
    >                   PT_S2                    CD4T                     Mes 
    >                     545                     566                     614 
    >                    Podo                     DCT                     PEC 
    >                     643                     732                     820 
    >                     CNT           VSMC/Pericyte                    Mono 
    >                     879                     892                     939 
    >                      IC                     Mac                  MyoFib 
    >                     969                    1099                    1121 
    >            Plasma_Cells                      PC              GS_Stromal 
    >                    1501                    1601                    2243 
    >                   PT_S1                     iPT                     Fib 
    >                    2289                    2851                    3183 
    >                   PT_S3                   C_TAL 
...
Multi-sample studies often require controlling for “sample effects”
while samples are nested within broader experimental conditions To
account for these effects without introducing multi-collinearity, we
assign a nested ID (1, 2, 3…) to each sample within its respective
condition.
...
The design matrix is constructed using the formula
`~ 0 + Group:celltype + Group:nested_id`. The `0 +` term removes the
global intercept, allowing the model to estimate absolute means for each
...
Raw design matrices often produce messy column names (e.g.,
GroupDKD:nested_id2). We rename these to reflect the actual sample IDs
...
```
    ...
    > [1] "GroupControl:celltypeC_TAL" "GroupDKD:celltypeC_TAL"    
...
- **Model Varieties:**
  - **Leroux** is our preferred default; it decomposes variance into
    spatially correlated and uncorrelated components and offers the
    fastest pre-computation.
  - **CAR and SAR** are also supported but are slightly more
    computationally demanding during the preparation phase.
- **Pre-computation:** These models require a one-time pre-computation
  of eigenvalues for specific sparse matrices. While this takes a few
  seconds per sample, it makes the subsequent gene-level model fitting
  faster.
...
a pre-computed adjacency matrix. However, if the adjacency matrix is
unknown, we provide internal utilities to determine cell-cell adjacency
...
cells within this distance as adjacent. Ultimately,
`expected_num_neighbors` serves to automate the discovery of a valid
`D_THRESH` argument for structured data, while `D_THRESH` itself offers
...
![](vignette_files/figure-html/visualize_neighbors-1.png)
...
To fit a model, you must provide three core inputs: the processed data
object, the specific gene name, and the desired covariance structure
(e.g., “Leroux”).

`TESSERA` uses an Expectation-Conditional Maximization (ECM) algorithm
to estimate model parameters. While the default settings are robust for
most spatial transcriptomics datasets, you can fine-tune the convergence
...
identifiable parameters `gamma_hat` or `tau2_hat`. While individual
spNNGP model parameters are not identifiable, their estimates are
provided in the relevant `nugget_hat`, `sill_hat`, `range_hat`, and
`smoothness_hat` columns, with `kernel_type` specifying the Gaussian
process kernel used.
...
as a critical diagnostic to ensure that the spatial structure has been
adequately captured, indicating that the model has successfully
...
```
    ...
    >   gene fit_model kernel_type contrast_indices contrast_val contrast_se
    > 1  A2M    Leroux          NA              1_2   -0.1190266  0.03726227
    > 2  A2M    Leroux          NA              1_3    0.4897356  0.06393596
    >   wald_stat_t contrast_description
...
```
where $`\beta_k`$ is the $`k^{th}`$ entry of the fixed effects vector
$`\beta`$ and the $`c_k`$ are coefficients that encode the contrasts.
...
We iterate through each pair, identifying the corresponding columns in
the design matrix. By assigning weights of $`1/3`$ (or $`1/n`$ depending
on the number of conditions), we ensure we are testing the difference in
...
Finally, we bind the individual rows together to create the full
contrast matrix.
...
```
    ...
    > C_TAL-CNT                   0.3333333              0.3333333
    >            GroupHKD:celltypeC_TAL GroupControl:celltypeCD4T
    > C_TAL-CD4T              0.3333333                -0.3333333
    > C_TAL-CNT               0.3333333                 0.0000000
    >            GroupDKD:celltypeCD4T GroupHKD:celltypeCD4T
    > C_TAL-CD4T            -0.3333333            -0.3333333
...
First, we identify the unique conditions and generate all possible
...
```
    ...
    >                   GroupControl:celltypeC_TAL GroupDKD:celltypeC_TAL
    > C_TAL:Control-DKD                          1                     -1
    > C_TAL:Control-HKD                          1                      0
    >                   GroupHKD:celltypeC_TAL GroupControl:celltypeCD4T
    > C_TAL:Control-DKD                      0                         0
    > C_TAL:Control-HKD                     -1                         0
    >                   GroupDKD:celltypeCD4T
    > C_TAL:Control-DKD                     0
    > C_TAL:Control-HKD                     0
...
To execute the full procedure, the following code can be used by setting
`eval = TRUE` in the code chunk header. Please note that the estimated
...
We can inspect the estimated scale and shift parameters for the fitted
...
![](vignette_files/figure-html/n_sig_plot-1.png)
...
key trade-offs to consider: `fdrtool` offers a faster runtime than
`TESSERA`’s thresholding procedure, but `TESSERA` makes fewer underlying
assumptions about the data distribution. Both methods achieve highly
comparable performance on this class of spatial transcriptomics data as
can be seen in the `TESSERA` methodological manuscript ([Constantine et
...
```
    ...
    >  abind                  1.4-8       [90m2024-09-12 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  Biobase              * 2.70.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.3) [39m [22m
    >  BiocFileCache        * 3.0.0       [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  BiocGenerics         * 0.56.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  BiocManager            1.30.27     [90m2025-11-14 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  BiocStyle            * 2.38.0      [90m2025-10-29 [39m  [90m[2] [39m  [90mBioconductor 3.22 (R 4.5.1) [39m
    >  bit                    4.6.0       [90m2025-03-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  bit64                  4.6.0-1     [90m2025-01-16 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  blob                   1.3.0       [90m2026-01-14 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  bookdown               0.46        [90m2025-12-05 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  BRISC                  1.0.6       [90m2024-10-02 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  bslib                  0.10.0      [90m2026-01-26 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  cachem                 1.1.0       [90m2024-05-16 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  cli                    3.6.6       [90m2026-04-09 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  colorspace             2.1-2       [90m2025-09-22 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  curl                   7.0.0       [90m2025-08-19 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  DBI                    1.3.0       [90m2026-02-25 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  dbplyr               * 2.5.2       [90m2026-02-13 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  DelayedArray           0.36.1      [90m2026-03-31 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.3) [39m [22m
    >  deldir                 2.0-4       [90m2024-02-28 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  desc                   1.4.3       [90m2023-12-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  devtools               2.5.0       [90m2026-03-14 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  digest                 0.6.39      [90m2025-11-19 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  dplyr                * 1.2.1       [90m2026-04-03 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  ellipsis               0.3.3       [90m2026-04-04 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  evaluate               1.0.5       [90m2025-08-27 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  farver                 2.1.2       [90m2024-05-13 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  fastmap                1.2.0       [90m2024-05-15 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  fdrtool              * 1.2.18      [90m2024-08-20 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  filelock               1.0.3       [90m2023-12-11 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  FNN                    1.1.4.1     [90m2024-09-22 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  fs                     2.0.1       [90m2026-03-24 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  generics             * 0.1.4       [90m2025-05-09 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  GenomicRanges        * 1.62.1      [90m2025-12-08 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  ggplot2              * 4.0.2       [90m2026-02-03 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  glue                   1.8.0       [90m2024-09-30 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  gstat                  2.1-6       [90m2026-03-30 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  gtable                 0.3.6       [90m2024-10-25 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  htmltools              0.5.9       [90m2025-12-04 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  htmlwidgets            1.6.4       [90m2023-12-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  httr2                  1.2.2       [90m2025-12-08 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  intervals              0.15.5      [90m2024-08-23 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  IRanges              * 2.44.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  jquerylib              0.1.4       [90m2021-04-26 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  jsonlite               2.0.0       [90m2025-03-27 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  knitr                  1.51        [90m2025-12-20 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  labeling               0.4.3       [90m2023-08-29 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  lattice                0.22-9      [90m2026-02-09 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  lifecycle              1.0.5       [90m2026-01-08 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  magick                 2.9.1       [90m2026-02-28 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  magrittr               2.0.5       [90m2026-04-04 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  MASS                 * 7.3-65      [90m2025-02-28 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  Matrix               * 1.7-5       [90m2026-03-21 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  MatrixGenerics       * 1.22.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  matrixStats          * 1.5.0       [90m2025-01-07 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  memoise                2.0.1       [90m2021-11-26 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  nloptr                 2.2.1       [90m2025-03-17 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  numDeriv                [1m [35m2016.8-1.1 [39m [22m  [90m2019-06-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  optimx                  [1m [35m2025-4.9   [39m [22m  [90m2025-04-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  otel                   0.2.0       [90m2025-08-29 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pbapply                1.7-4       [90m2025-07-20 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pillar                 1.11.1      [90m2025-09-17 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pkgbuild               1.4.8       [90m2025-05-26 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pkgconfig              2.0.3       [90m2019-09-22 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pkgdown                2.2.0       [90m2025-11-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pkgload                1.5.1       [90m2026-04-01 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  plyr                   1.8.9       [90m2023-10-02 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  Polychrome             1.5.4       [90m2025-04-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  polyclip               1.10-7      [90m2024-07-23 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  pracma                 2.4.6       [90m2025-10-22 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  purrr                  1.2.2       [90m2026-04-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  R6                     2.6.1       [90m2025-02-15 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  ragg                   1.5.2       [90m2026-03-23 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  RANN                   2.6.2       [90m2024-08-25 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  rappdirs               0.3.4       [90m2026-01-17 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  RColorBrewer           1.1-3       [90m2022-04-03 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  Rcpp                   1.1.1       [90m2026-01-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  RcppParallel           5.1.11-2    [90m2026-03-05 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  rdist                  0.0.5       [90m2020-05-04 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  reshape2             * 1.4.5       [90m2025-11-12 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  Rfast                  2.1.5.2     [90m2025-10-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  rjson                  0.2.23      [90m2024-09-16 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  rlang                  1.2.0       [90m2026-04-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  rmarkdown              2.31        [90m2026-03-26 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  RSQLite                2.4.6       [90m2026-02-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  rstudioapi             0.18.0      [90m2026-01-16 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  S4Arrays               1.10.1      [90m2025-12-01 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  S4Vectors            * 0.48.1      [90m2026-04-05 [39m  [90m[2] [39m  [90mBioconductor 3.22 (R 4.5.1) [39m
    >  S7                     0.2.1       [90m2025-11-14 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  sass                   0.4.10      [90m2025-04-11 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  scales                 1.4.0       [90m2025-04-24 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  scatterplot3d          0.3-45      [90m2026-02-23 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  Seqinfo              * 1.0.0       [90m2025-10-29 [39m  [90m[2] [39m  [90mBioconductor 3.22 (R 4.5.1) [39m
    >  sessioninfo            1.2.3       [90m2025-02-05 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  SingleCellExperiment * 1.32.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  sp                     2.2-1       [90m2026-02-13 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  spacetime              1.3-3       [90m2025-02-13 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  SparseArray            1.10.10     [90m2026-03-30 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.3) [39m [22m
    >  sparseMatrixStats      1.22.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  SpatialExperiment    * 1.20.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  spatstat.data          3.1-9       [90m2025-10-18 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  spatstat.geom          3.7-3       [90m2026-03-23 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  spatstat.univar        3.1-7       [90m2026-03-18 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  spatstat.utils         3.2-2       [90m2026-03-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  stringi                1.8.7       [90m2025-03-27 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    ...
    >  usethis                3.2.1       [90m2025-09-06 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  vctrs                  0.7.3       [90m2026-04-11 [39m  [90m[2] [39m  [90mCRAN (R 4.5.1) [39m
    >  withr                  3.0.2       [90m2024-10-28 [39m  [90m[2] [39m  [90mCRAN (R 4.5.0) [39m
    >  xfun                   0.57        [90m2026-03-20 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  xts                    0.14.2      [90m2026-02-28 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
    >  XVector                0.50.0      [90m2025-10-29 [39m  [90m[2] [39m  [1m [35mhttps://bioc-release.r-universe.dev (R 4.5.2) [39m [22m
    >  yaml                   2.3.12      [90m2025-12-10 [39m  [90m[2] [39m  [90mCRAN (R 4.5.2) [39m
...
<https://doi.org/10.1038/s41588-024-01802-x>.

Constantine, Florica J., Zoltan Lasik, Sandrine Dudoit, and Elizabeth
Purdom. 2026. “Unlocking Multi-Sample Differential Expression for
Spatial Transcriptomics Data with TESSERA.” *bioRxiv*.

Efron, Bradley. 2004. “Large-Scale Simultaneous Hypothesis Testing: The
...
Strimmer, Korbinian. 2008. “Fdrtool: A Versatile r Package for
Estimating Local and Tail Area-Based False Discovery Rates.”
*Bioinformatics* 24 (12): 1461–62.
