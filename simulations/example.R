# Example of running poisECM algorithm
...
source("./R/M_step.R")
source("./R/models.R")
source("./R/poisECM_spNNGP.R")
source("./R/poisECM_lattice.R")
source("./R/prepData.R")
source("./R/utils.R")
...
# Create design matrix
X_mat <- model.matrix(design_formula, seu_kid@meta.data)
...
k_search <- 20
dist_out <- visualizeNeighborDistances(
...
### Prepare data for poisECM algorithm ###

prep_out <- prepData(
  count_matrix = seu_kid[["RNA"]]$counts,
  meta_data = seu_kid@meta.data,
  sample_col = sample_col,
  design_mat = X_mat,
  coord_data = seu_kid@meta.data[, coord_cols],
...
### Run poisECM algorithm ###

alg_out <- poisECM_lattice(
...
alg_out_spnngp <- poisECM_spNNGP(
  poisECMData_obj = prep_out_spnngp,
  gene_name = "B2M",
  cov_type = "Exp",
  nngp_k = 5,
  em_iters = 2,
